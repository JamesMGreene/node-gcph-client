...
var Client = function(opts) {
	...
	// This number is the absolute maximum number of issues/comments that will be queried for.
	// If you do NOT want a maximum upper bound, then do NOT set this.
	if (typeof opts.maxResults === 'number') {
		this.options.maxResults = opts.maxResults;
	}

	// Counters for `maxResults` enforcement
	this._remainingQuotas = {
		issues: null,
		comments: null
	};
};
...
var createAnonymousClient = function(authedClient) {
	...
	if (typeof authedClient.options.maxResults === 'number') {
		anonOptions.maxResults = authedClient.options.maxResults;
	}
...
Client.prototype.getIssues = function(project, query, done) {
    ...
    var origQuery = query ? JSON.parse(JSON.stringify(query)) : null;
				...
				function(err, gcIssues) {
					me._remainingQuotas.issues = null;
	...
	if (!project) {
		...
		if (typeof me.options.maxResults === 'number') {
			// Reset the quota. When the request is finished, the quota will be set to `null`.
			if (typeof me._remainingQuotas.issues !== 'number') {
				me._remainingQuotas.issues = me.options.maxResults;
			}
			var remainingQuota = me._remainingQuotas.issues;
			var desiredMaxResults = parseInt(query['max-results'], 10) || MAX_RESULTS_ALLOWED_BY_GOOGLE;
			if (desiredMaxResults > remainingQuota) {
				query['max-results'] = remainingQuota;
				console.warn('WARNING: You are requesting up to ' + desiredMaxResults + ' issues but will only receive up to ' +
					remainingQuota + ' due to the options you specified!\n' +
					'Please modify the `maxResults` option (currently ' + me.options.maxResults +
					') on the `Client` instance if this behavior is not acceptable.');
			}
			else {
				query['max-results'] = desiredMaxResults;
			}
		}
		else {
			query['max-results'] = query['max-results'] || MAX_RESULTS_ALLOWED_BY_GOOGLE;
		}
					...
					try {
						...
						if (atom && atom.feed &&
							...
							if (nextUrls.length > 1) {
								...
								me._remainingQuotas.issues -= atom.feed.entry.length;
								...
								// Chain!
								var nextStartId = parseInt(url.parse(nextUrls[0], true).query['start-index'], 10);
								var currentMax = query['max-results'] + (query['start-index'] || 0);
								if (!isNaN(nextStartId) && nextStartId > 0 && nextStartId > currentMax) {
									query['start-index'] = nextStartId;
									process.nextTick(function() {
										me.getIssues(project, query, nextDone);
									});
								}
								else {
									done(new Error('Google Code returned a "next" URL with an invalid start index.'));
								}
							}
							else {
								me._remainingQuotas.issues -= atom.feed.entry.length;
...
Client.prototype.getComments = function(project, issueId, done) {
	...
	if (!project) {
		...
		var query = new Query({
			'alt': JSON_FEED_TYPE,
			'max-results': MAX_RESULTS_ALLOWED_BY_GOOGLE
		});
		var queryParamsString = (function() {
			var qs = Query.stringify(query);
			return qs ? '?' + qs : '';
								...
								function(err /*, gcComments */) {
									if (err) {
										...
										me.anonClient.getComments(project, issueId, function(err, gcObfuscatedComments) {
											postFixCommentsWithPrivateEmails(gcObfuscatedComments, me.uriToPrivateEmailMap, done);
										});
									}
								};
