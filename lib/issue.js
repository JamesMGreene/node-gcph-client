...
*
*/
Issue.prototype.toRawIssueXml = function() {
...
'  <issues:label>' + xmlEncode(label) + '</issues:label>'
	...
	if (this.owner) {
		rawIssueXmlBuilder.splice.apply(rawIssueXmlBuilder, [rawIssueXmlBuilder.length, 0, 
'  <issues:owner>',
...
'  </issues:cc>'
	...
	rawIssueXmlBuilder.push('</entry>');
	
	return rawIssueXmlBuilder.join('\n');
};
